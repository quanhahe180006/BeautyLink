...
public class AvailabilityService {
    ...
    public AvailabilityResponse availability(Long serviceId, Long practitionerId, LocalDate date) {
        if (date.isBefore(LocalDate.now())) throw new ApiException(HttpStatus.BAD_REQUEST, "PAST_DATE", "Không thể đặt lịch trong quá khứ");
        ...
        for (LocalTime cursor = start; !cursor.plusMinutes(service.getDurationMinutes()).isAfter(end); cursor = cursor.plusMinutes(step)) {
            ...
            boolean alreadyPast = date.equals(LocalDate.now()) && slotStart.isBefore(LocalTime.now().plusMinutes(30));
